...
const navItems = [
  ...
  { to: '/borrower/lenders', label: 'Find Lender', icon: Search },
...
export function BorrowerShell() {
  ...
  return (
    <div className="flex h-screen bg-background">
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-56 flex-shrink-0 flex-col bg-white border-r border-border shadow-sm">
          ...
          {navItems.map(({ to, label, icon: Icon, end }) => (
            <NavLink key={to} to={to} end={end}
              className={({ isActive }) => cn(
                ...
                isActive ? 'bg-primary text-white shadow-sm' : 'text-muted-foreground hover:bg-accent hover:text-foreground'
              )}
            >
