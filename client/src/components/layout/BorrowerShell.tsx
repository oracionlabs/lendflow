...
import { LayoutDashboard, PlusCircle, FileText, User, BarChart2, LogOut } from 'lucide-react'
import { cn } from '@/lib/utils'

const navItems = [
  { to: '/borrower', label: 'Dashboard', icon: LayoutDashboard, end: true },
  { to: '/borrower/apply', label: 'Apply for Loan', icon: PlusCircle },
  { to: '/borrower/loans', label: 'My Loans', icon: FileText },
  { to: '/borrower/reports', label: 'Reports', icon: BarChart2 },
  { to: '/borrower/profile', label: 'Profile', icon: User },
]
    ...
    <div className="flex h-screen bg-background">
      <aside className="w-56 flex-shrink-0 flex flex-col bg-white border-r border-border shadow-sm">
        <div className="px-5 py-5 border-b border-border">
          ...
          <p className="text-[11px] text-muted-foreground mt-1 ml-9">Borrower Portal</p>
        </div>
        ...
        <div className="px-3 py-3 border-t border-border">
          <div className="flex items-center gap-2.5 px-3 py-2 rounded-lg mb-0.5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-muted-foreground truncate">Borrower</p>
            </div>
          </div>
          <button
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 bg-white border-b border-border flex items-center justify-between px-6 shadow-sm">
          <h2 className="text-sm font-semibold text-foreground">{currentPage?.label ?? 'Dashboard'}</h2>
